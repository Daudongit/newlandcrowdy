...
class Referrals {
  ...
  async credit(user_id, amount) {
    const userReferral = await Referral.query().where({
      user_id: user_id
    }).first();

    if (userReferral && userReferral.status == 0) {

      const referralsPercentage = (await Reference.query().where('slug', 'referrals_percent').first()).value;
      ...
      }).increment('wallet', amountForReferrals).then(() => {})

      Referral.query().where({
        id: userReferral.id
      }).update({
        amount: amountForReferrals,
        status: 1
