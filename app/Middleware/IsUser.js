 ...
 class IsUser {
   async handle({
     ...
     if (
       request.url().indexOf('supports') === -1 && auth.user.suspended === 1
     ) {
         session.flash({
     ...
     if ([Route.url('app.account.settings'), Route.url('app.account.profile')].indexOf(request.url()) === -1) {
     ...
     if ([
       ...
       if (auth.user.active == 0) {
         const amount = (await Reference.query().where('slug', 'activation_amount').first()).value;
         session.flash({
